...
resource "google_compute_region_backend_service" "default" {
  ...
  dynamic "backend" {
    ...
    content {
      ...
      max_connections              = lookup(backend.value, "max_connections", 0)
      max_connections_per_endpoint = lookup(backend.value, "max_connections_per_endpoint", 0)
      max_connections_per_instance = lookup(backend.value, "max_connections_per_instance", 0)
      max_rate                     = lookup(backend.value, "max_rate", 0)
      max_rate_per_endpoint        = lookup(backend.value, "max_rate_per_endpoint", 0)
      max_rate_per_instance        = lookup(backend.value, "max_rate_per_instance", 0)
      max_utilization              = lookup(backend.value, "max_utilization", 0)
  ...
  dynamic "outlier_detection" {
    for_each = each.value.outlier_detection != null && (var.load_balancing_scheme == "INTERNAL_SELF_MANAGED" || var.load_balancing_scheme == "INTERNAL_MANAGED" || var.load_balancing_scheme == "EXTERNAL_MANAGED") ? [1] : []
    content {
